...
public:
    ...
    SoftUart(gpio_num_t rx_pin, gpio_num_t tx_pin, uint32_t speed, bool invert) :
        rx_pin(rx_pin), tx_pin(tx_pin), speed(speed), invert(invert), state(State::Idle) {
            ...
            if (!input_q) {
                ESP_LOGE(TAG, "could not create queue. panicking!");
                abort();
            }
    ...
    bool transmit(uint8_t byte) {
        ESP_LOGD(TAG, "sending byte %02X", byte);
        ...
        if (!xSemaphoreTake(this->tx_flag, pdMS_TO_TICKS(500))) {
            ESP_LOGE(TAG, "transmission of byte %X never succeeded", byte);
            return false;
        }
            ...
            case State::Idle:
                {
                    ...
                    hw_timer_set_load_data(uart->hw_timer_load_data >> 4);
                    hw_timer_enable(true);
                    */
