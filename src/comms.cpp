...
void setup_comms() {
    ...
    gdoSecurityType = read_gdo_security_from_flash("gdo_security");
...
void comms_loop() {
    ...
    if (gdoSecurityType == 1) {
        ...
        if (gotMessage) {
            ...
            else if (key == 0x32) { RINFO("0x32 RX (light press)"); }
            else if (key == 0x33) { RINFO("0x33 RX (light release)"); }

            // 2 byte status messages (0x38 - 0x3A)
            if (key >= 0x38 && key <= 0x3A) {
